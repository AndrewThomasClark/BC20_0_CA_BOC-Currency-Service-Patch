...
codeunit 80988 "PTE-ATCTMP Update FCY"
{
    ...
    var
        ...
        MyHttpResponse: HttpResponseMessage;
    begin
        CurrExchRateUpdateSetup.GetWebServiceURL(URL);

        MyHttpClient.Get(URL, MyHttpResponse);
        ...
        If not MyHttpResponse.IsSuccessStatusCode() then
            Error(Format(MyHttpResponse.HttpStatusCode()) + ' ' + MyHttpResponse.ReasonPhrase());

        MyHttpResponse.Content.ReadAs(ResponseInStream);
    ...
    var
        ...
        MapCurrencyExchangeRate: Codeunit "PTE-ATCTMP Map FCY Rate";
        ResponseInStream: InStream;
        SourceName: Text;
    begin
        GetCurrencyExchangeData(CurrExchRateUpdateSetup, ResponseInStream, SourceName);
